...
impl Support {
    pub fn init(window_dimensions: (u32, u32)) -> Support {
    ...
    pub fn render<G, F: FnMut(&Ui, &mut G)>(&mut self, clear_color: (f32, f32, f32, f32), game: &mut G, mut run_ui: F) {
        let now = Instant::now();
    ...
    pub fn update_events(&mut self) -> bool {
        for event in self.display.poll_events() {
            match event {
                ...
                Event::KeyboardInput(game, _, code) => {
                    let pressed = game == ElementState::Pressed;
                    match code {
                ...
                Event::MouseInput(game, MouseButton::Left) => {
                    self.mouse_pressed.0 = game == ElementState::Pressed
                }
                Event::MouseInput(game, MouseButton::Right) => {
                    self.mouse_pressed.1 = game == ElementState::Pressed
                }
                Event::MouseInput(game, MouseButton::Middle) => {
                    self.mouse_pressed.2 = game == ElementState::Pressed
                }
